...
all:
	make -C $(SRC_PATH) -f firmware.mk VARIANT=$(VARIANT) VARIANT_PATH=$(VARIANT_PATH) OUT=$(MAKE_OUT) PLATFORM=$(PLATFORM) CPU=$(CPU) CHIP=$(CHIP) BOARD=$(BOARD) SOCNAME=$(SOCNAME) $(EXTRA_ARGS) $(TARGET)
ifdef IMAGE_OUT
	cd $(VARIANT_ABSPATH) && \
...
clean:
	rm -rf $(VARIANT_ABSPATH)/$(SRC_PATH)/$(OUT)
ifdef IMAGE_OUT
