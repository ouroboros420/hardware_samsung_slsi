...
void spiBoardInit(void)
{
    ...
    for(i=0; i<num; i++){
        ...
        else if (portNum == SPI_CMGP0)  spiType=CMU_CLK_CMGP_SPI0;
        else if (portNum == SPI_CMGP1)  spiType=CMU_CLK_CMGP_SPI1;
        else if (portNum == SPI_CMGP2)  spiType=CMU_CLK_CMGP_SPI2;
        else if (portNum == SPI_CMGP3)  spiType=CMU_CLK_CMGP_SPI3;

        inputClock = cmuGetSpeed(spiType);
        clockDivider = (inputClock + (mSpiDevConf[i].m_OpClock * 4) - 1) / (mSpiDevConf[i].m_OpClock * 4);
        CSP_PRINTF_INFO("Set SPI(%lu) Clock : in = %lu , div = %lu\n", portNum, inputClock, clockDivider);
