...
#define LSM6DSM_SPI_SLAVE_BUS_ID    ACC_GYR_SPI_BUS_ID
#define LSM6DSM_INT1_GPIO           ACC_GYR_INT_PIN1
#define LSM6DSM_SPI_SLAVE_FREQUENCY_HZ ACC_GYR_SPI_CLOCK
#define LSM6DSM_SPI_SLAVE_CS_GPIO   GPH1_3
#define LSM6DSM_INT_IRQ
#define LSM6DSM_ACCEL_GYRO_ROT_MATRIX  ACCEL_GYRO_ROT_MATRIX
...
static bool lsm6dsm_isr1(struct ChainedIsr *isr)
{
    ...
    gpioClearPending(LSM6DSM_INT1_GPIO);
#else
...
{
#ifdef EXYNOS_CONTEXTHUB
    gpioUnsetExtInterrupt(LSM6DSM_INT1_GPIO);
#else
...
static bool lsm6dsm_updateOdrs(void)
{
    ...
    /* Disable Fifo. Trigger every data */
...
{
    TDECL();
    struct LSM6DSMAccelGyroCfgData *cfgData = data + 4;
...
static void lsm6dsm_handleSpiDoneEvt(const void *evtData)
{
    ...
    case SENSOR_TIME_SYNC: ;
        ...
        if (T(time).status == TIME_SYNC_TIMER) {
            T(time).syncTimerId = timTimerSet(LSM6DSM_SYNC_DELTA_INTERVAL, 100, 100, lsm6dsm_timerSyncCallback, NULL, true);
            if (T(time).syncTimerId == 0)
...
#ifdef EXYNOS_CONTEXTHUB
    ipc_set_sensor_info(SENSOR_TYPE_GYROSCOPE, "LSM6DSM Gyro", VENDOR_NAME_ST, 0, 0);
    ipc_set_sensor_info(SENSOR_TYPE_GYROSCOPE_UNCALIBRATED, "LSM6DSM Gyro(uncal)", VENDOR_NAME_ST, 0, 0);
    ipc_set_sensor_info(SENSOR_TYPE_ACCELEROMETER, "LSM6DSM Accel", VENDOR_NAME_ST, 0, 0);
    ipc_set_sensor_info(SENSOR_TYPE_ACCELEROMETER_UNCALIBRATED, "LSM6DSM Accel(uncal)", VENDOR_NAME_ST, 0, 0);
#endif
    return true;
}
