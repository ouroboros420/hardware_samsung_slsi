...
#define DATA_SLAB_3AXIS_MAX_NUM    3
...
static void ak09918_dumpRegister(void)
{
    ...
    DEBUG_PRINT("REG_HXL ~ REG_HZH\nx(%f), y(%f), z(%f)\n",
            (double)(*(int16_t*)(&mTask.txrxBuf[0])),
            (double)(*(int16_t*)(&mTask.txrxBuf[2])),
            (double)(*(int16_t*)(&mTask.txrxBuf[4])));
...
static bool ak09918_initApp(uint32_t taskId)
{
    ...
    mTask.dataSlab3Axis = slabAllocatorNew(slabSize, 4, DATA_SLAB_3AXIS_MAX_NUM);

#if defined(AK09918_CAL_ENABLED)
    mag_cal_parameters.min_batch_window_in_micros = 3000000;
...
static void ak09918_freeAxisData(void* event)
{
    DEBUG_PRINT("%s,%d:free %p\n", __func__, __LINE__, event);
    if(mTask.dataSlab3Axis && event)
        slabAllocatorFree(mTask.dataSlab3Axis, event);
    else
        ERROR_PRINT("free one axis data failed.(%p/%p)\n", mTask.dataSlab3Axis, event);
}
...
static bool ak09918_pushRawData(void)
{
    ...
    uint8_t regSt1;
    uint8_t regSt2;
    struct TripleAxisDataEvent* tAxisData = NULL;

    tAxisData = (struct TripleAxisDataEvent*)slabAllocatorAlloc(mTask.dataSlab3Axis);
    if(!tAxisData){
        ERROR_PRINT("No free space for allocting one axisData.\n");
        return false;
    }
    ...
    if(mTask.lastTime == 0){
        tAxisData->samples[0].deltaTime = 0;
    }else{
        if( (rtcTime - mTask.lastTime) < MIN_DELTA_TIME )
        {
            slabAllocatorFree(mTask.dataSlab3Axis, tAxisData);
            return false;
        }
        tAxisData->samples[0].deltaTime = rtcTime - mTask.lastTime;
    }

    mTask.lastTime = rtcTime;
    tAxisData->referenceTime = rtcTime;
    memset(&tAxisData->samples[0].firstSample, 0x00, sizeof(struct SensorFirstSample));
    tAxisData->samples[0].firstSample.numSamples = 1;
    ...
    tAxisData->samples[0].x = (int16_t)(mTask.txrxBuf[2] << 8) + (int16_t)(mTask.txrxBuf[1]);
    tAxisData->samples[0].y = (int16_t)(mTask.txrxBuf[4] << 8) + (int16_t)(mTask.txrxBuf[3]);
    tAxisData->samples[0].z = (int16_t)(mTask.txrxBuf[6] << 8) + (int16_t)(mTask.txrxBuf[5]);

    // change ak09918 axis to adapt android axis
    tAxisData->samples[0].x = -tAxisData->samples[0].x *0.15 ;
    tAxisData->samples[0].y = -tAxisData->samples[0].y *0.15 ;
    tAxisData->samples[0].z = tAxisData->samples[0].z *0.15 ;

     AKM09918_TO_ANDROID_COORDINATE(tAxisData->samples[0].x,
		tAxisData->samples[0].y,
		tAxisData->samples[0].z);

    DEBUG_PRINT("mag_data x, y, z: %f, %f, %f\n",
            (double)tAxisData->samples[0].x,
            (double)tAxisData->samples[0].y,
            (double)tAxisData->samples[0].z);
    ...
    x = tAxisData->samples[0].x;
    y = tAxisData->samples[0].y;
    z = tAxisData->samples[0].z;

    magCalRemoveSoftiron(&mTask.moc, x, y, z, &biasX, &biasY, &biasZ);
    newMagCalibData = magCalUpdate(&mTask.moc, TIME_NS_TO_US(mTask.lastTime), biasX, biasY, biasZ);
    magCalRemoveBias(&mTask.moc, biasX, biasY, biasZ, &x, &y, &z);

    tAxisData->samples[0].x = x;
    tAxisData->samples[0].y = y;
    tAxisData->samples[0].z = z;

    if (newMagCalibData) {
        tAxisData->samples[1].deltaTime = 0;
        magCalGetBias(&mTask.moc,
        &tAxisData->samples[1].x,
        &tAxisData->samples[1].y,
        &tAxisData->samples[1].z);

        tAxisData->samples[0].firstSample.numSamples = 2;
        tAxisData->samples[0].firstSample.biasCurrent = true;
        tAxisData->samples[0].firstSample.biasPresent = 1;
        tAxisData->samples[0].firstSample.biasSample = 1;
    }
#endif
    return osEnqueueEvt(sensorGetMyEventType(SENS_TYPE_MAG), tAxisData, ak09918_freeAxisData);
}
