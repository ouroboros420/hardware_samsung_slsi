...
#define BMI160_SPI_BUS_ID         ACC_GYR_SPI_BUS_ID
#define BMI160_SPI_SPEED_HZ       8000000
#define BMI160_SPI_MODE           3

//#define BMI160_INT_IRQ            SYSREG1_CMGP_IRQn
#define BMI160_INT1_PIN           ACC_GYR_GPIO_PIN1
#define BMI160_INT1_EINT          ACC_GYR_INT_PIN1
#define BMI160_INT2_PIN           ACC_GYR_GPIO_PIN2
#define BMI160_INT2_EINT          ACC_GYR_INT_PIN2
...
#define MAX_NUM_COMMS_EVENT_SAMPLES 15
    ...
    #if defined(SUPPORT_HIFI_SENSORS)
        #define BMI160_ACC_SAMPLES 3000
    #else
        #define BMI160_ACC_SAMPLES 1000
    #endif
#endif

#ifndef BMI160_GYRO_SAMPLES
#define BMI160_GYRO_SAMPLES 200
#endif
...
#define kMinRTCTimeIncrementNs   312500ull // forced min rtc time increment, 312.5us for 1600Hz
#else
#define kMinRTCTimeIncrementNs   1250000ull // forced min rtc time increment, 1.25ms for 400Hz
#endif

#if defined(SUPPORT_1600HZ)
#define kMinSensorTimeIncrement  16          // forced min sensortime increment,
#else
...
struct BMI160Task {
    ...
    uint8_t samplesToDiscard; //BST code: samples to discard
...
static bool bmi160Isr1(struct ChainedIsr *isr)
{
    ...
    gpioClearPending(BMI160_INT1_PIN);
#else
...
static bool bmi160Isr2(struct ChainedIsr *isr)
{
    ...
    gpioClearPending(BMI160_INT2_PIN);
#else
...
static bool enableInterrupt(struct Gpio *pin, struct ChainedIsr *isr)
{
    ...
    //extiSetIntGpio(pin, EXTI_TRIGGER_RISING, isr);
#else
...
{
#ifdef EXYNOS_CONTEXTHUB
    gpioUnsetExtInterrupt(BMI160_INT1_EINT);
  #if (SUPPORT_ACC_GYR_INT_PIN2)
    gpioUnsetExtInterrupt(BMI160_INT2_EINT);
  #endif
#else
...
static bool accSetRate(uint32_t rate, uint64_t latency, void *cookie)
{
    ...
    //if (rate >= SENSOR_HZ(200))
    //    dfsSetFreqLock(SENS_TYPE_ACCEL, DFS_FREQ_180MHz);
    ...
    if (trySwitchState(SENSOR_CONFIG_CHANGING)) {
        ...
        mTask.samplesToDiscard = 2; //BST Code: set number of samples to discard
...
static bool gyrSetRate(uint32_t rate, uint64_t latency, void *cookie)
{
    ...
    //if (rate >= SENSOR_HZ(200))
    //    dfsSetFreqLock(SENS_TYPE_GYRO, DFS_FREQ_180MHz);
...
static void parseRawData(struct BMI160Sensor *mSensor, uint8_t *buf, float kScale, uint64_t sensorTime)
{
    ...
    int num_of_smpl = 1;  //BST Code
#ifdef MAG_SLAVE_PRESENT
    bool newMagBias = false;
#endif

    //BST Code: Discard samples generated during sensor turn-on time after rate change.
...
static void sensorInit(void)
{
    ...
    case INIT_ON_CHANGE_SENSORS:
        ...
        ipc_set_sensor_info(SENSOR_TYPE_ACCELEROMETER, NULL, NULL, SENS_TYPE_ACCEL, BMI160_ID);
        ipc_set_sensor_info(SENSOR_TYPE_GYROSCOPE, NULL, NULL, SENS_TYPE_GYRO, BMI160_ID);
...
static void handleSpiDoneEvt(const void* evtData)
{
    ...
    case SENSOR_SAVE_CALIBRATION:
        INFO_PRINT("SENSOR_SAVE_CALIBRATION: %02x %02x %02x %02x %02x %02x %02x\n",
...
#ifdef EXYNOS_CONTEXTHUB
    T(Eint1) = gpioRequest(BMI160_INT1_EINT);
    T(Isr1).func = bmi160Isr1;
  #if (SUPPORT_ACC_GYR_INT_PIN2)
    T(Eint2) = gpioRequest(BMI160_INT2_EINT);
...
#ifdef EXYNOS_CONTEXTHUB
    enableInterrupt(T(Eint1), &T(Isr1));
  #if (SUPPORT_ACC_GYR_INT_PIN2)
    enableInterrupt(T(Eint2), &T(Isr2));
  #endif

    ipc_set_sensor_info(SENSOR_TYPE_GYROSCOPE, "BMI160 Gyro", VENDOR_NAME_BOSCH, 0, 0);
    ipc_set_sensor_info(SENSOR_TYPE_GYROSCOPE_UNCALIBRATED, "BMI160 Gyro(uncal)", VENDOR_NAME_BOSCH, 0, 0);
    ipc_set_sensor_info(SENSOR_TYPE_ACCELEROMETER, "BMI160 Accel", VENDOR_NAME_BOSCH, 0, 0);
    ipc_set_sensor_info(SENSOR_TYPE_ACCELEROMETER_UNCALIBRATED, "BMI160 Accel(uncal)", VENDOR_NAME_BOSCH, 0, 0);
#else
...
static void endTask(void)
{
    ...
    gpioRelease(mTask.Eint1);
    gpioRelease(mTask.Int2);
    gpioRelease(mTask.Eint2);
#else
