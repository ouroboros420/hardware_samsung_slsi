...
static inline void allowRamExecution(bool allow)
{
    mpuAllowRamExecution(allow);
}

static inline void allowRomWrite(bool allow)
{
    mpuAllowRomWrite(allow);
}
...
#ifndef EXYNOS_LOG_TO_KERNEL
    rtcSetTickTime(3, 0, 0, logbuf_callback, 0);
#endif
...
void osLog(enum LogLevel level, const char *str, ...)
{
    ...
    va_start(vl, str);
    platLogv((char)level, 0, str, vl);
    va_end(vl);
}

#if 0
void osLog_NoTimeStamp(const char *str, ...)
{
    va_list vl;
#ifdef EXYNOS_CONTEXTHUB
    void *userData = platLogAllocUserData(0);
#else
    void *userData = platLogAllocUserData();
#endif

    va_start(vl,str);
    cvprintf(platLogPutcharF, 0, userData, str, vl);
