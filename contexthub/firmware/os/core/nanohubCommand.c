...
#ifndef DEBUG_PRINT
#ifdef PACKET_DEBUG
#define DEBUG_PRINT(fmt, ...) osLog(fmt, ##__VA_ARGS__)
#else
...
static uint32_t getImageHash(void *rx, uint8_t rx_len, void *tx, uint64_t timestamp)
{
    struct NanohubOsHashImageRequest *req = rx;
    struct NanohubOsHashImageResponse *resp = tx;
    struct Sha2state sha;
    char *outSha2;

    osLog(LOG_INFO, "%s: start:0x%x, end:0x%x, size:%d\n",
	    __func__, (u32)req->start, (u32)req->end,
	    (u32)(req->end - req->start));
    sha2init(&sha);
    sha2processBytes(&sha, (void *)req->start, req->end - req->start);
    outSha2 = (char *)sha2finish(&sha);
    memcpy(outSha2, &resp->hash[0], sizeof(resp->hash));

    osLog(LOG_INFO, "%s: result:size:%d-%d, hash:%x, %x, %x, %x\n",
	    __func__, sizeof(resp->hash), sizeof(*resp),
	    resp->hash[0], resp->hash[1], resp->hash[2], resp->hash[3]);
    return sizeof(*resp);
}
    ...
    while (hostIntfPacketDequeue(&mTxNext, wakeup, nonwakeup)) {
        ...
        if (packet->sensType == SENS_TYPE_INVALID) {
            ...
            case HOSTINTF_DATA_TYPE_APP_TO_SENSOR_HAL:
                ...
                break;
            case HOSTINTF_DATA_TYPE_DFS_TO_SENSOR_HAL:
                packet->evtType = htole32(EVT_DFS_TO_SENSOR_HAL);
                break;
#ifdef DEBUG_LOG_EVT
...
#if defined(EXYNOS_CONTEXTHUB) && defined(DEF_IPC_TIMESTAMP)
    resp->chubTime = getTimeStampUS();
    resp->padding = 0xeeeeffff;
#endif
...
const static struct NanohubCommand mBuiltinCommands[] = {
    NANOHUB_COMMAND(NANOHUB_REASON_GET_OS_HASH,
                    getImageHash,
                    getImageHash,
                    struct NanohubOsHashImageRequest,
                    struct NanohubOsHashImageResponse),
