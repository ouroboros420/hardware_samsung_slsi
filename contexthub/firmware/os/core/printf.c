...
static uint32_t StrPrvPrintfEx_number(printf_write_c putc_, struct PrintfData *data, bool *bail)
{
    ...
    *bail = false;
    ...
    if (data->flags & FLAG_IS_SIGNED) {
        ...
        if ((data->flags & FLAG_ZERO_NEGATIVE) || (((int64_t)data->number) < 0)) {
...
#undef GET_SVAL64
                ...
                case 'f':
                   ...
                   if(useLongDouble) {
                        ...
                        data.number = floatToInt64(dbl);
                        data.base = 10;
                        data.flags &= ~(FLAG_ALT | FLAG_CAPS);
                        data.flags |= FLAG_IS_SIGNED;
                        if (data.number == 0 &&  dbl < (double) 0.0f)
                            data.flags |= FLAG_ZERO_NEGATIVE;
                        numPrinted += StrPrvPrintfEx_number(putc_f, &data, &bail);
                        ...
                        dbl -= (double) floatFromInt64(floatToInt64(dbl));

                        data.fieldWidth = data.precision;
                        while (data.precision--)
                            dbl *= 10;
                        data.precision = data.fieldWidth;
