...
#define CHECK_LATENCY_TIME      100000000   /* ns */
...
static inline void hostIntfIntSet(bool on)
{
    apIntSet(on);
}

static inline void hostIntfIntClear(bool on)
{
    apIntClear(on);
}
...
bool hostIntfPacketDequeue(void *data, uint32_t *wakeup, uint32_t *nonwakeup)
{
    ...
    if (!ret) {
        ...
        for (i = 0; i < mNumSensors; i++, mLastSensor = (mLastSensor + 1) % mNumSensors) {
            ...
            if (sensor->curSamples != sensor->buffer.firstSample.numSamples) {
                osLog(LOG_ERROR, "hostIntfPacketDequeue(%d) %d != %d len=%d %lu %lu\n",
                    sensor->buffer.sensType, sensor->curSamples, sensor->buffer.firstSample.numSamples,
                    sensor->buffer.length, mWakeupBlocks, mNonWakeupBlocks);
    ...
    if (ret) {
        sensor = getActiveSensorByType(buffer->sensType);
        if (sensor) {
            if (sensor->interrupt == NANOHUB_INT_WAKEUP && mWakeupBlocks)
                mWakeupBlocks--;
            else if (sensor->interrupt == NANOHUB_INT_NONWAKEUP && mNonWakeupBlocks)
                mNonWakeupBlocks--;
            sensor->curSamples -= buffer->firstSample.numSamples;
            sensor->firstTime = 0ull;
        } else {
            if (buffer->interrupt == NANOHUB_INT_WAKEUP && mWakeupBlocks)
                mWakeupBlocks--;
            else if (buffer->interrupt == NANOHUB_INT_NONWAKEUP && mNonWakeupBlocks)
                mNonWakeupBlocks--;
        }
    }
...
static bool initSensors()
{
    ...
    dfsInit(mNumSensors, mSensorList);
#else
...
static void onEvtAppStart(const void *evtData)
{
    if (initSensors()) {
        ...
        osEventsSubscribe(5, EVT_NO_SENSOR_CONFIG_EVENT,
                             EVT_APP_TO_SENSOR_HAL_DATA,
                             EVT_APP_TO_HOST,
                             EVT_DFS_TO_SENSOR_HAL,
                             EVT_SENSOR_HAL_TO_DFS,
    ...
    for (i = 0, cnt = 0; i < mNumSensors && cnt < mLatencyCnt; i++) {
        if (mActiveSensorTable[i].latency > 0) {
            ...
            if (mActiveSensorTable[i].firstTime &&
                ...
                //if (!((ipc_get_ap_wake() == AP_SLEEP) && (mActiveSensorTable[i].interrupt == NANOHUB_INT_NONWAKEUP)))
...
#if defined(DFS_SUPPORT)
static void onEvtSensorHalToDfs(const void *evtData) {
    struct DfsToSensorHalDataPayload* data = (struct DfsToSensorHalDataPayload*)evtData;
    dfsLoadSampleTime(data);
}

static void onEvtDfsToSensorHal(const void *evtData)
{
    struct HostIntfDataBuffer *data = (struct HostIntfDataBuffer *)evtData;
    if (data->sensType == SENS_TYPE_INVALID
            && data->dataType == HOSTINTF_DATA_TYPE_DFS_TO_SENSOR_HAL) {
        hostIntfAddBlock(data, false, false);
    }
}
#endif
...
static void hostIntfHandleEvent(uint32_t evtType, const void* evtData)
{
    ...
    case EVT_DFS_TO_SENSOR_HAL:
        onEvtDfsToSensorHal(evtData);
        break;
    case EVT_SENSOR_HAL_TO_DFS:
        onEvtSensorHalToDfs(evtData);
        break;
#endif
    default:
...
void hostIntfClearInterrupt(uint32_t bit)
{
    ...
    if (mHostIntfTid) {
        if (atomicBitsetGetBit(mInterrupt, bit)) {
            ...
            if (!atomicBitsetGetBit(mInterruptMask, bit)) {
                if (--mInterruptCntWkup == 0)
                    hostIntfIntClear(true);
            } else {
                if (--mInterruptCntNonWkup == 0)
                    hostIntfIntClear(false);
            }
        }
    }
    cpuIntsRestore(state);
