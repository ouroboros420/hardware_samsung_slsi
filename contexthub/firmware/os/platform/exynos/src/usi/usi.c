...
void usiSaveState(void)
{
    ...
    for (i = 0 ; i < USI_CHUB_CHANNEL_MAX ; i++){
        if (mUsiInfo[i].state == USI_STATE_OPEN)
            mUsiInfo[i].state = USI_STATE_REOPEN;
    }
}
...
void usiRestoreState(void)
{
    ...
    for (i = 0 ; i < USI_CHUB_CHANNEL_MAX ; i++){
        if (mUsiInfo[i].state == USI_STATE_REOPEN)
...
{
    UsiProtocolType protocol = usiGetProtocol(port);

    switch (protocol) {
    case USI_PROTOCOL_SPI:
        spi_IRQHandler(mUsiInfo[port].ch_spi);
        break;
    case USI_PROTOCOL_I2C:
        i2c_IRQHandler(mUsiInfo[port].ch_i2c);
        break;
    case USI_PROTOCOL_UART:
        // NOT support yet
        break;
