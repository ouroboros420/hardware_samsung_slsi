...
static UsiInfoType mUsiInfo[I2C_CHANNEL_MAX] = {
    ...
    DEC_USI(USI1,  SYSREG_BASE_ADDRESS + 0x2010, SPI_CHUB1, I2C_CHUB1),
    DEC_USI(USI2,  SYSREG_BASE_ADDRESS + 0x2014,         0, I2C_CHUB2),
    DEC_USI(USI3,  SYSREG_BASE_ADDRESS + 0x2020, SPI_CHUB2, I2C_CHUB3),
    DEC_USI(USI4,  SYSREG_BASE_ADDRESS + 0x2030, SPI_CHUB3, I2C_CHUB4),
    DEC_USI(USI5,  SYSREG_BASE_ADDRESS + 0x2034,         0, I2C_CHUB5),
...
void usi4_IRQHandler(void) {
    ...
    if (protocol == USI_PROTOCOL_I2C) {
        ...
        spi_IRQHandler(SPI_CHUB3);
    }
}

void usi5_IRQHandler(void) {
    i2c_IRQHandler(I2C_CHUB5);
}

void usi6_IRQHandler(void) {
    uint32_t protocol;

    protocol = __raw_readl(mUsiInfo[6].base + REG_USI_CONFIG);
    if (protocol == USI_PROTOCOL_I2C) {
...
void usi8_IRQHandler(void) {
    ...
    protocol = __raw_readl(mUsiInfo[8].base + REG_USI_CONFIG);
    if (protocol == USI_PROTOCOL_I2C) {
...
void usi10_IRQHandler(void) {
    ...
    protocol = __raw_readl(mUsiInfo[10].base + REG_USI_CONFIG);
    if (protocol == USI_PROTOCOL_I2C) {
...
void usi12_IRQHandler(void) {
    ...
    protocol = __raw_readl(mUsiInfo[12].base + REG_USI_CONFIG);
    if (protocol == USI_PROTOCOL_I2C) {
...
void usi14_IRQHandler(void) {
    ...
    protocol = __raw_readl(mUsiInfo[14].base + REG_USI_CONFIG);
    if (protocol == USI_PROTOCOL_I2C) {
