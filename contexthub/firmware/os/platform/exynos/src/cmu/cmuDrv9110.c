...
static void cmuDrvSetMuxCmgpUSI00(uint32_t sel);
static void cmuDrvSetMuxCmgpUSI01(uint32_t sel);
static void cmuDrvSetMuxCmgpUSI02(uint32_t sel);
static void cmuDrvSetMuxCmgpUSI03(uint32_t sel);
static void cmuDrvSetMuxCmgpI2C00(uint32_t sel);
static void cmuDrvSetMuxCmgpI2C01(uint32_t sel);
static void cmuDrvSetMuxCmgpI2C02(uint32_t sel);
static void cmuDrvSetMuxCmgpI2C03(uint32_t sel);
...
static void cmuDrvSetMuxCmgpUSI00(IN uint32_t sel)
{
    uint32_t regValue;

    regValue = __raw_read32(REG_CMU_CLK_CON_MUX_MUX_CLK_CMGP_USI0) & 0xFFFFFFFE;
    regValue = regValue | (sel & 0x1);
    __raw_write32(regValue, REG_CMU_CLK_CON_MUX_MUX_CLK_CMGP_USI0);
}

static void cmuDrvSetMuxCmgpUSI01(IN uint32_t sel)
{
    uint32_t regValue;

    regValue = __raw_read32(REG_CMU_CLK_CON_MUX_MUX_CLK_CMGP_USI1) & 0xFFFFFFFE;
    regValue = regValue | (sel & 0x1);
    __raw_write32(regValue, REG_CMU_CLK_CON_MUX_MUX_CLK_CMGP_USI1);
}

static void cmuDrvSetMuxCmgpUSI02(IN uint32_t sel)
{
    uint32_t regValue;

    regValue = __raw_read32(REG_CMU_CLK_CON_MUX_MUX_CLK_CMGP_USI2) & 0xFFFFFFFE;
    regValue = regValue | (sel & 0x1);
    __raw_write32(regValue, REG_CMU_CLK_CON_MUX_MUX_CLK_CMGP_USI2);
}

static void cmuDrvSetMuxCmgpUSI03(IN uint32_t sel)
{
    uint32_t regValue;

    regValue = __raw_read32(REG_CMU_CLK_CON_MUX_MUX_CLK_CMGP_USI3) & 0xFFFFFFFE;
    regValue = regValue | (sel & 0x1);
    __raw_write32(regValue, REG_CMU_CLK_CON_MUX_MUX_CLK_CMGP_USI3);
}

static void cmuDrvSetMuxCmgpI2C00(IN uint32_t sel)
{
    uint32_t regValue;

    regValue = __raw_read32(REG_CMU_CLK_CON_MUX_MUX_CLK_CMGP_I2C0) & 0xFFFFFFFE;
    regValue = regValue | (sel & 0x1);
    __raw_write32(regValue, REG_CMU_CLK_CON_MUX_MUX_CLK_CMGP_I2C0);
}

static void cmuDrvSetMuxCmgpI2C01(IN uint32_t sel)
{
    uint32_t regValue;

    regValue = __raw_read32(REG_CMU_CLK_CON_MUX_MUX_CLK_CMGP_I2C1) & 0xFFFFFFFE;
    regValue = regValue | (sel & 0x1);
    __raw_write32(regValue, REG_CMU_CLK_CON_MUX_MUX_CLK_CMGP_I2C1);
}

static void cmuDrvSetMuxCmgpI2C02(IN uint32_t sel)
{
    uint32_t regValue;

    regValue = __raw_read32(REG_CMU_CLK_CON_MUX_MUX_CLK_CMGP_I2C2) & 0xFFFFFFFE;
    regValue = regValue | (sel & 0x1);
    __raw_write32(regValue, REG_CMU_CLK_CON_MUX_MUX_CLK_CMGP_I2C2);
}

static void cmuDrvSetMuxCmgpI2C03(IN uint32_t sel)
{
    uint32_t regValue;

    regValue = __raw_read32(REG_CMU_CLK_CON_MUX_MUX_CLK_CMGP_I2C3) & 0xFFFFFFFE;
    regValue = regValue | (sel & 0x1);
...
void cmuDrvInit(uint32_t mainclk)
{
    ...
    cmuDrvSetMuxCmgpUSI00(0);
    cmuDrvSetMuxCmgpUSI01(0);
    cmuDrvSetMuxCmgpUSI02(0);
    cmuDrvSetMuxCmgpUSI03(0);
    cmuDrvSetMuxCmgpI2C00(0);
    cmuDrvSetMuxCmgpI2C01(0);
    cmuDrvSetMuxCmgpI2C02(0);
    cmuDrvSetMuxCmgpI2C03(0);
