...
static uint32_t cmuDrvGetSpeedMuxChubDmailbox(void);
...
//static void cmuDrvSetDivChubuPMU_TEST(IN uint32_t divider);
...
//  static void cmuDrvGetDivChubuPMU_TEST(void);
...
#if defined (EVT0)
static uint32_t cmuDrvGetSpeedMuxCmgpI3c(void);
#endif
...
#if defined (EVT0)
static void cmuDrvSetMuxClkCmgpI3C(int);
#endif
...
#if defined (EVT0)
static void cmuDrvSetDivCmgpI3C(IN uint32_t divider);
#endif
...
#if defined (EVT0)
static uint32_t cmuDrvGetDivCmgpI3c(void);
#endif
...
#if defined (EVT0)
    {REG_CMU_CLK_CON_MUX_MUX_CLK_CHUB_DMAILBOX, 0},
#elif defined (EVT1)
...
#if defined (EVT0)
    {REG_CMU_CLK_CON_DIV_DIV_CLK_CHUB_DMAILBOX, 0},
#endif
};
...
void cmuDrvInit(uint32_t mainClk)
{
    ...
    cmuDrvSetMuxClkAliveChubNocUser(CLKALIVE_CHUB_NOC);
    cmuDrvSetMuxClkAliveChubPeriUser(CLKALIVE_CHUB_PERI);
    cmuDrvSetMuxClkAliveChubRcoUser(CLKALIVE_CHUB_RCO);
...
#if defined(SEOS)
static void __attribute__((unused)) cmuDrvSetMuxClkChubDMAILBOX(int select) {
#else
static void cmuDrvSetMuxClkChubDMAILBOX(int select) {
    ...
    uint32_t regAddr;
#if defined (EVT0)
    regAddr = REG_CMU_CLK_CON_MUX_MUX_CLK_CHUB_DMAILBOX;
#elif defined (EVT1)
    regAddr = REG_CMU_CLK_CON_MUX_MUX_CLK_CHUBVTS_DMAILBOX_CCLK;
#endif
    regValue = __raw_read32(regAddr);
    ...
    __raw_write32(regValue, regAddr);

    // Wait while mux is changing
    while(__raw_read32(regAddr) & MUX_BUSY);
}
...
static uint32_t cmuDrvGetSpeedMuxChubNocUser(void)
{
    return (__raw_read32(REG_CMU_PLL_CON0_MUX_CLKALIVE_CHUB_NOC_USER) & MUX_USER_SEL)? HFRCO_FREQ : CLK_RCO_FREQ;
}

static uint32_t cmuDrvGetSpeedMuxChubPeriUser(void)
{
    return (__raw_read32(REG_CMU_PLL_CON0_MUX_CLKALIVE_CHUB_PERI_USER) & MUX_USER_SEL)? HFRCO_FREQ : CLK_RCO_FREQ;
}

static uint32_t cmuDrvGetSpeedMuxChubRcoUser(void)
{
    return (__raw_read32(REG_CMU_PLL_CON0_MUX_CLKALIVE_CHUB_RCO_USER) & MUX_USER_SEL)? HFRCO_FREQ : CLK_RCO_FREQ;
}
...
{
    return (__raw_read32(REG_CMU_PLL_CON0_MUX_CLKALIVE_CHUBVTS_NOC_USER) & MUX_USER_SEL)? HFRCO_FREQ : CLK_RCO_FREQ;
}

static uint32_t cmuDrvGetSpeedMuxChubVtsRcoUser(void)
{
    return (__raw_read32(REG_CMU_PLL_CON0_MUX_CLKALIVE_CHUBVTS_RCO_USER) & MUX_USER_SEL)? HFRCO_FREQ : CLK_RCO_FREQ;
}
#endif

#if defined(SEOS)
static uint32_t __attribute__((unused)) cmuDrvGetSpeedMuxChubDmailbox(void) {
#else
static uint32_t cmuDrvGetSpeedMuxChubDmailbox(void) {
#endif
    uint32_t speed = 0;
    ...
    if(__raw_read32(REG_CMU_CLK_CON_MUX_MUX_CLK_CHUB_DMAILBOX) & MUX_SEL)
        ...
        speed = cmuDrvGetSpeedMuxChubRcoUser();
#elif defined(EVT1)
    if(__raw_read32(REG_CMU_CLK_CON_MUX_MUX_CLK_CHUBVTS_DMAILBOX_CCLK) & MUX_SEL)
        speed = cmuDrvGetSpeedMuxChubVtsNocUser();
    else
        speed = cmuDrvGetSpeedMuxChubVtsRcoUser();
#endif
...
static void cmuDrvSetDivChubNoc(IN uint32_t divider)
{
    ...
    __raw_write32(((__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CHUB_NOC) & ~DIV_RATIO_MASK_3BIT) | divider), REG_CMU_CLK_CON_DIV_DIV_CLK_CHUB_NOC);
...
static void cmuDrvSetDivChubuPMU_TEST(IN uint32_t divider)
{
    ...
    // divider value is clear and set to given divider value
    divider = (divider - 1) & DIV_RATIO_MASK_3BIT;
    __raw_write32(((__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CHUB_NOC) & ~DIV_RATIO_MASK_3BIT) | divider), REG_CMU_CLK_CON_DIV_DIV_CLK_CHUB_BUS);
...
static void cmuDrvSetDivChubUSI3(IN uint32_t divider)
{
    ...
    // divider value is clear and set to given divider value
    divider = (divider - 1) & DIV_RATIO_MASK_5BIT;
    __raw_write32(((__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CHUB_USI3) & ~DIV_RATIO_MASK_4BIT) | divider), REG_CMU_CLK_CON_DIV_DIV_CLK_CHUB_USI3);
...
static void cmuDrvSetDivChubSPI_MS_CTRL(IN uint32_t divider)
{
    ...
    // divider value is clear and set to given divider value
    divider = (divider - 1) & DIV_RATIO_MASK_4BIT;
    __raw_write32(((__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CHUB_SPI_MS_CTRL) & ~DIV_RATIO_MASK_4BIT) | divider), REG_CMU_CLK_CON_DIV_DIV_CLK_CHUB_SPI_MS_CTRL);
...
#if defined (EVT0)
    divider = (divider - 1) & DIV_RATIO_MASK_4BIT;
    regAddr = REG_CMU_CLK_CON_DIV_DIV_CLK_CHUB_DMAILBOX;
    __raw_write32(((__raw_read32(regAddr) & ~DIV_RATIO_MASK_4BIT) | divider), regAddr);
#elif defined (EVT1)
    divider = (divider - 1) & DIV_RATIO_MASK_5BIT;
    regAddr = REG_CMU_CLK_CON_DIV_DIV_CLK_CHUBVTS_DMAILBOX_CCLK;
    __raw_write32(((__raw_read32(regAddr) & ~DIV_RATIO_MASK_5BIT) | divider), regAddr);
#endif

    // Wait while divider is changing
    while(__raw_read32(regAddr) & DIV_BUSY);
}
...
static void cmuDrvSetDivChubSPII2C0(IN uint32_t divider)
{
    ...
    // divider value is clear and set to given divider value
    divider = (divider - 1) & DIV_RATIO_MASK_4BIT;
    __raw_write32(((__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CHUB_SPI_I2C0) & ~DIV_RATIO_MASK_4BIT) | divider), REG_CMU_CLK_CON_DIV_DIV_CLK_CHUB_SPI_I2C0);
...
static void cmuDrvSetDivChubSPII2C1(IN uint32_t divider)
{
    ...
    // divider value is clear and set to given divider value
    divider = (divider - 1) & DIV_RATIO_MASK_4BIT;
    __raw_write32(((__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CHUB_SPI_I2C1) & ~DIV_RATIO_MASK_4BIT) | divider), REG_CMU_CLK_CON_DIV_DIV_CLK_CHUB_SPI_I2C1);
...
{
    uint32_t divider;
    divider = (__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CHUB_NOC) & DIV_RATIO_MASK_3BIT) + 1;
...
{
    uint32_t divider;
    divider = (__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CHUB_SPI_MS_CTRL) & DIV_RATIO_MASK_4BIT) + 1;
...
static uint32_t cmuDrvGetDivChubDmailbox(void)
{
    uint32_t divider;
#if defined (EVT0)
    divider = (__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CHUB_DMAILBOX) & DIV_RATIO_MASK_4BIT) + 1;
#elif defined (EVT1)
    divider = (__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CHUBVTS_DMAILBOX_CCLK) & DIV_RATIO_MASK_5BIT) + 1;
#endif
...
{
    uint32_t divider;
    divider = (__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CHUB_SPI_I2C0) & DIV_RATIO_MASK_4BIT) + 1;
...
{
    uint32_t divider;
    divider = (__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CHUB_SPI_I2C1) & DIV_RATIO_MASK_4BIT) + 1;
...
#if defined (EVT0)
#if defined(SEOS)
static void __attribute__((unused)) cmuDrvSetMuxClkCmgpI3C(int select) {
#else
static void cmuDrvSetMuxClkCmgpI3C(int select) {
#endif
    uint32_t regValue;
    regValue = __raw_read32(REG_CMU_CLK_CON_MUX_MUX_CLK_CMGP_I3C);
    ...
    __raw_write32(regValue, REG_CMU_CLK_CON_MUX_MUX_CLK_CMGP_I3C);

    // Wait while mux is changing
    while(__raw_read32(REG_CMU_CLK_CON_MUX_MUX_CLK_CMGP_I3C) & MUX_BUSY);
}
#endif
...
#if defined (EVT0)
#if defined(SEOS)
static uint32_t __attribute__((unused)) cmuDrvGetSpeedMuxCmgpI3c(void) {
#else
static uint32_t cmuDrvGetSpeedMuxCmgpI3c(void) {
#endif
    uint32_t speed;

    if(__raw_read32(REG_CMU_CLK_CON_MUX_MUX_CLK_CMGP_I3C) & MUX_SEL)
        speed = HFRCO_FREQ;
    else
        speed = CLK_RCO_FREQ;
...
#if defined (EVT0)
#if defined(SEOS)
static void __attribute__((unused)) cmuDrvSetDivCmgpI3C(IN uint32_t divider) {
#else
static void cmuDrvSetDivCmgpI3C(IN uint32_t divider) {
#endif
    if (divider == 0)
    ...
    // divider value is clear and set to given divider value
    divider = (divider - 1) & DIV_RATIO_MASK_4BIT;
    __raw_write32(((__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CMGP_I3C) & ~DIV_RATIO_MASK_4BIT) | divider),
                  REG_CMU_CLK_CON_DIV_DIV_CLK_CMGP_I3C);

    // Wait while divider is changing
    while(__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CMGP_I3C) & DIV_BUSY);
}
#endif
...
static void cmuDrvSetDivCmgpSPI_I2C0(IN uint32_t divider) {
    ...
    // divider value is clear and set to given divider value
    divider = (divider - 1) & DIV_RATIO_MASK_4BIT;
    __raw_write32(((__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CMGP_SPI_I2C0) & ~DIV_RATIO_MASK_4BIT) | divider),
...
static void cmuDrvSetDivCmgpSPI_I2C1(IN uint32_t divider) {
    ...
    // divider value is clear and set to given divider value
    divider = (divider - 1) & DIV_RATIO_MASK_4BIT;
    __raw_write32(((__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CMGP_SPI_I2C1) & ~DIV_RATIO_MASK_4BIT) | divider),
...
static void cmuDrvSetDivCmgpSPI_MS_CTRL(IN uint32_t divider) {
    ...
    // divider value is clear and set to given divider value
    divider = (divider - 1) & DIV_RATIO_MASK_4BIT;
    __raw_write32(((__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CMGP_SPI_MS_CTRL) & ~DIV_RATIO_MASK_4BIT) | divider),
...
static void cmuDrvSetDivCmgpUSI5(IN uint32_t divider) {
    ...
    // divider value is clear and set to given divider value
    divider = (divider - 1) & DIV_RATIO_MASK_5BIT;
    __raw_write32(((__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CMGP_USI5) & ~DIV_RATIO_MASK_4BIT) | divider),
...
static void cmuDrvSetDivCmgpUSI6(IN uint32_t divider) {
    ...
    // divider value is clear and set to given divider value
    divider = (divider - 1) & DIV_RATIO_MASK_5BIT;
    __raw_write32(((__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CMGP_USI6) & ~DIV_RATIO_MASK_4BIT) | divider),
...
#if defined (EVT0)
#if defined(SEOS)
static uint32_t __attribute__((unused)) cmuDrvGetDivCmgpI3c(void) {
#else
static uint32_t cmuDrvGetDivCmgpI3c(void) {
#endif
    uint32_t divider;
    divider = (__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CMGP_I3C) & DIV_RATIO_MASK_4BIT) + 1;
...
static uint32_t cmuDrvGetDivCmgpSpiI2c0(void) {
    uint32_t divider;
    divider = (__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CMGP_SPI_I2C0) & DIV_RATIO_MASK_4BIT) + 1;
...
static uint32_t cmuDrvGetDivCmgpSpiI2c1(void) {
    uint32_t divider;
    divider = (__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CMGP_SPI_I2C1) & DIV_RATIO_MASK_4BIT) + 1;
...
static uint32_t cmuDrvGetDivCmgpSpiMsCtrl(void) {
    uint32_t divider;
    divider = (__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CMGP_SPI_MS_CTRL) & DIV_RATIO_MASK_4BIT) + 1;
...
static uint32_t cmuDrvGetDivCmgpUsi5(void) {
    uint32_t divider;
    divider = (__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CMGP_USI5) & DIV_RATIO_MASK_4BIT) + 1;
...
static uint32_t cmuDrvGetDivCmgpUsi6(void) {
    uint32_t divider;
    divider = (__raw_read32(REG_CMU_CLK_CON_DIV_DIV_CLK_CMGP_USI6) & DIV_RATIO_MASK_4BIT) + 1;
...
uint32_t cmuDrvGetSpeed(IN CmuIpType ip)
{
    switch(ip) {
...
#endif
        case CMU_MUX_CHUB_DMAILBOX:
            return cmuDrvGetSpeedMuxChubDmailbox();
...
#if defined (EVT0)
            return cmuDrvGetSpeedMuxChubNocUser() / cmuDrvGetDivChubNoc();
#elif defined (EVT1)
            return cmuDrvGetSpeedMuxChubNoc() / cmuDrvGetDivChubNoc();
#endif
        case CMU_DIV_CHUB_DMAILBOX:
            return cmuDrvGetSpeedMuxChubDmailbox() / cmuDrvGetDivChubDmailbox();
        ...
        //case CMU_MUX_CMGP_NOC_USER:
        //    return cmuDrvGetSpeedMuxCmgpNocUser();
        //case CMU_MUX_CMGP_PERI_USER:
        //    return cmuDrvGetSpeedMuxCmgpPeriUser();
        case CMU_MUX_CMGP_I2C:
            return cmuDrvGetSpeedMuxCmgpI2c();
#if defined (EVT0)
        case CMU_MUX_CMGP_I3C:
            return cmuDrvGetSpeedMuxCmgpI3c();
#endif
...
#if defined (EVT0)
        case CMU_DIV_CMGP_I3C:
        case CMU_CLK_CMGP_I3C0:
            return cmuDrvGetSpeedMuxCmgpI3c() / cmuDrvGetDivCmgpI3c();
#endif
