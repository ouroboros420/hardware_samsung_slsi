...
#define ALM_INT_BIT     ALM_INT_MASK
...
static void (*mAlarmCallback)(void);

static void rtcEnableControl(void)
{
    uint32_t regValue;

    regValue = __raw_readl(REG_RTC_RTCCON);
    regValue |= (0x1 << CTLEN_BIT);
    __raw_writel(regValue, REG_RTC_RTCCON);
}

static void rtcDisableControl(void)
{
    uint32_t regValue;
...
void rtcInit(void)
{
    ...
    // LATCH Enable
    regValue = __raw_readl(REG_RTC_RTCCON);
    __raw_writel(regValue | 0x100, REG_RTC_RTCCON);
    __raw_writel(regValue & (~0x100), REG_RTC_RTCCON);
...
{
    return __raw_readl(REG_RTC_TS_CURTICCNT);
}

static inline uint32_t dec2bcd(uint32_t dec)
{
    uint32_t hval;
    ...
    return hval + ((dec / 10) << 4) + (dec % 10);
}

static inline uint32_t bcd2dec(uint32_t bcd)
{
    uint32_t hval;
                    ...
                    uint32_t hour, uint32_t min, uint32_t sec)
{
    ...
    __raw_writel(YYY, REG_RTC_BCDYEAR);
    __raw_writel(MM,  REG_RTC_BCDMON);
    __raw_writel(DD,  REG_RTC_BCDDAY);
    __raw_writel(hh,  REG_RTC_BCDHOUR);
    __raw_writel(mm,  REG_RTC_BCDMIN);
    __raw_writel(ss,  REG_RTC_BCDSEC);
...
{
    rtcSetDateTime(time.tm_year, time.tm_mon + 1, time.tm_mday, time.tm_hour, time.tm_min, time.tm_sec);

    rtcEnableControl();
    __raw_writel(time.tm_wday,  REG_RTC_BCDDAYWEEK);
    rtcDisableControl();
}

void rtcGetTime64(union time64_t *time)
{
    time->tm_year = bcd2dec(__raw_readl(REG_RTC_BCDYEAR));
    time->tm_mon = bcd2dec(__raw_readl(REG_RTC_BCDMON)) - 1;
    time->tm_mday = bcd2dec(__raw_readl(REG_RTC_BCDDAY));
    time->tm_hour = bcd2dec(__raw_readl(REG_RTC_BCDHOUR));
    time->tm_min = bcd2dec(__raw_readl(REG_RTC_BCDMIN));
    time->tm_sec = bcd2dec(__raw_readl(REG_RTC_BCDSEC));
    time->tm_wday = __raw_readl(REG_RTC_BCDDAYWEEK);
}

void rtcGetDateTime(uint32_t* YYYY, uint32_t* MM, uint32_t* DD,
                    uint32_t* hh, uint32_t* mm, uint32_t* ss)
{
    *YYYY = 1900 + bcd2dec(__raw_readl(REG_RTC_BCDYEAR));
    *MM = bcd2dec(__raw_readl(REG_RTC_BCDMON));
    *DD = bcd2dec(__raw_readl(REG_RTC_BCDDAY));
    *hh = bcd2dec(__raw_readl(REG_RTC_BCDHOUR));
    *mm = bcd2dec(__raw_readl(REG_RTC_BCDMIN));
    *ss = bcd2dec(__raw_readl(REG_RTC_BCDSEC));
}
                         ...
                         uint32_t hour, uint32_t min, uint32_t sec)
{
    ...
    __raw_writel(YYY, REG_RTC_ALMYEAR);
    __raw_writel(MM,  REG_RTC_ALMMON);
    __raw_writel(DD,  REG_RTC_ALMDAY);
    __raw_writel(hh,  REG_RTC_ALMHOUR);
    __raw_writel(mm,  REG_RTC_ALMMIN);
    __raw_writel(ss,  REG_RTC_ALMSEC);
}
...
{
    uint32_t regValue = (0x1 << ALMEN_BIT); // Global Alarm Enable

    if (en_year)
        regValue |= (0x1 << YEAREN_BIT);
    if (en_mon)
        regValue |= (0x1 << MONEN_BIT);
    if (en_date)
        regValue |= (0x1 << DAYEN_BIT);
    if (en_hour)
        regValue |= (0x1 << HOUREN_BIT);
    if (en_min)
        regValue |= (0x1 << MINEN_BIT);
    if (en_sec)
        regValue |= (0x1 << SECEN_BIT);

    __raw_writel(regValue,  REG_RTC_RTCALM);
}
...
void rtcStartTSTick(void)
{
    ...
    regValue = __raw_readl(REG_RTC_TS_TICCON);
    regValue |= (0x1 << TS_TICKEN_BIT);
    __raw_writel(regValue, REG_RTC_TS_TICCON);
}
...
void rtc_IRQHandler(void)
{
    ...
    regValue = __raw_readl(REG_RTC_INTP);

    if (regValue & TICK_INT_BIT) {
        rtcStopTick();
    ...
    if (regValue & ALM_INT_BIT) {
        if (mAlarmCallback != NULL)
            mAlarmCallback();

        // Pending Clear
        __raw_writel(ALM_INT_BIT, REG_RTC_INTP);
    }
}
