...
#elif defined (s5e9610)
  #define CHUB_SHMIPC_TX_BUFFER_ADDRESS ((uint8_t *)0x0003FC00)
  #define CHUB_SHMIPC_TX_BUFFER_LENGTH  (0x400 - 0x18) /* -128 for the shared control registers area */

  #define CHUB_SHMIPC_RX_BUFFER_ADDRESS ((uint8_t *)0x0003F400)
  #define CHUB_SHMIPC_RX_BUFFER_LENGTH  (0x800)

  /*Guard Area - GNSS may use this area.*/
  #define CHUB_SHMIPC_RESERVED_BUFFER_ADDRESS ((uint8_t *)0x0003F000)
  #define CHUB_SHMIPC_RESERVED_BUFFER_LENGTH  (0x400)

  #define CHUB_SHMIPC_CONTROL_REGISTERS_ADDRESS    (0x0003FFE8)
#elif defined (s5e9925)
  #define CHUB_SHMIPC_TX_BUFFER_ADDRESS ((uint8_t *)0x0017FC00)
  #define CHUB_SHMIPC_TX_BUFFER_LENGTH  (0x400 - 0x18) /* -128 for the shared control registers area */

  #define CHUB_SHMIPC_RX_BUFFER_ADDRESS ((uint8_t *)0x0017F400)
  #define CHUB_SHMIPC_RX_BUFFER_LENGTH  (0x800)

  /*Guard Area - GNSS may use this area.*/
  #define CHUB_SHMIPC_RESERVED_BUFFER_ADDRESS ((uint8_t *)0x0017F000)
  #define CHUB_SHMIPC_RESERVED_BUFFER_LENGTH  (0x400)

  #define CHUB_SHMIPC_CONTROL_REGISTERS_ADDRESS    (0x0017FFE8)
#else
  #define CHUB_SHMIPC_TX_BUFFER_ADDRESS ((uint8_t *)(GNSS_SHMIPC_BASE_ADDRESS + 0xC00))
  #define CHUB_SHMIPC_TX_BUFFER_LENGTH  (0x400 - 0x18) /* -128 for the shared control registers area */

  #define CHUB_SHMIPC_RX_BUFFER_ADDRESS ((uint8_t *)(GNSS_SHMIPC_BASE_ADDRESS + 0x400))
  #define CHUB_SHMIPC_RX_BUFFER_LENGTH  (0x800)

  /*Guard Area - GNSS may use this area.*/
  #define CHUB_SHMIPC_RESERVED_BUFFER_ADDRESS ((uint8_t *) (GNSS_SHMIPC_BASE_ADDRESS))
  #define CHUB_SHMIPC_RESERVED_BUFFER_LENGTH  (0x400)

  #define CHUB_SHMIPC_CONTROL_REGISTERS_ADDRESS    (GNSS_SHMIPC_BASE_ADDRESS + 0xFE8)
#endif
...
void shmipc_init(void)
{
     ...
     CSP_PRINTF_INFO("%s:CHUB_SHMIPC_RX_HEAD(0x%x) = 0x%x\n",__func__,CHUB_SHMIPC_RX_HEAD,__raw_readl(CHUB_SHMIPC_RX_HEAD));
     CSP_PRINTF_INFO("%s:CHUB_SHMIPC_RX_TAIL(0x%x) = 0x%x\n",__func__,CHUB_SHMIPC_RX_TAIL,__raw_readl(CHUB_SHMIPC_RX_TAIL));
     CSP_PRINTF_INFO("%s:CHUB_SHMIPC_TX_HEAD(0x%x) = 0x%x\n",__func__,CHUB_SHMIPC_TX_HEAD,__raw_readl(CHUB_SHMIPC_TX_HEAD));
     CSP_PRINTF_INFO("%s:CHUB_SHMIPC_TX_TAIL(0x%x) = 0x%x\n",__func__,CHUB_SHMIPC_TX_TAIL,__raw_readl(CHUB_SHMIPC_TX_TAIL));
     CSP_PRINTF_INFO("%s:CHUB_SHMIPC_TX_IPC_MSG(0x%x) = 0x%x\n",__func__,CHUB_SHMIPC_TX_IPC_MSG,__raw_readl(CHUB_SHMIPC_TX_IPC_MSG));
     CSP_PRINTF_INFO("%s:CHUB_SHMIPC_RX_IPC_MSG(0x%x) = 0x%x\n",__func__,CHUB_SHMIPC_RX_IPC_MSG,__raw_readl(CHUB_SHMIPC_RX_IPC_MSG));

#if defined (s5e8535) || defined (s5e8835)
     CSP_PRINTF_INFO("Set Interrupt Mask!!");
     __raw_writel(0xFFF8,MAILBOX_GNSS_BASE_ADDRESS + 0x24);
#endif
