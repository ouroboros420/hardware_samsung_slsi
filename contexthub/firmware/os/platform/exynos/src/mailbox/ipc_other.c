...
void mailboxABOXHandleIRQ(int evt, void *priv)
{
	...
	if (cipc->cb) {
		if (evt == CIPC_REG_DATA_ABOX2CHUB || evt == CIPC_REG_DATA_ABOX2CHUB_AUD || evt == CIPC_REG_DATA_CHUB2ABOX) {
			ipc_buf = cipc_read_data((enum cipc_region)evt, &len);
                      #if defined(SEOS)
			local_buf = heapAlloc(len);
                      #elif defined(SEOS)
                        local_buf = OS_malloc(len);
                      #endif
			if (!local_buf) {
...
struct cipc_info *chub_cipc_init(enum ipc_owner owner, void *sram_base, struct cipc_funcs *func)
{
