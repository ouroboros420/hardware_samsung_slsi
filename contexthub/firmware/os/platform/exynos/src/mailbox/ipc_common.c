...
static unsigned int cipc_get_offset(enum cipc_region area)
{
	...
	return (unsigned int)((unsigned int)cipc.cipc_addr[area].base -
			      (unsigned int)cipc.cipc_addr[CIPC_REG_SRAM_BASE].base);
}

static unsigned int cipc_get_offset_addr(void *addr)
{
	return (unsigned int)((unsigned int)addr - (unsigned int)cipc.cipc_addr[CIPC_REG_SRAM_BASE].base);
}
...
static int is_cipc_valid(void)
{
	...
	for (i = 0; i < cipc.user_cnt; i++) {
		...
		if (is_user_valid((enum cipc_user_id)i) == CIPC_FALSE)
			return CIPC_FALSE;

		reg = (enum cipc_region)user_info->map_info.evt_reg;
		...
		for (j = 0; j < user_info->map_info.data_pool_cnt; j++) {
			reg = (enum cipc_region)user_info->map_info.data_reg[j];
...
static void print_cipc_map(void)
{
	...
	struct cipc_user_map_info *cipc_user_ival = (struct cipc_user_map_info *)(
		cipc.chub_bootargs->cipc_init_map + (unsigned int)cipc.sram_base);
	...
	for (i = 0; i < CIPC_REG_MAX; i++)
		CIPC_PRINT("%s(+%x %d)\n", cipc.cipc_addr[i].name,
			   cipc_get_offset((enum cipc_region)i), cipc_get_size((enum cipc_region)i));
	...
	for (i = 0; i < cipc.user_cnt; i++) {
		...
		CIPC_PRINT("reg(%d) (+%x, %d(remain_size:%d))\n",
			   user_map_info->reg,
			   cipc_get_offset((enum cipc_region)user_map_info->reg),
			   cipc_get_size((enum cipc_region)user_map_info->reg),
			   cipc_get_size((enum cipc_region)user_map_info->reg) -
			   cipc_get_size((enum cipc_region)user_map_info->evt_reg) -
			   cipc_get_size((enum cipc_region)user_map_info->data_reg[0]) -
			   cipc_get_size((enum cipc_region)user_map_info->data_reg[1]));
		CIPC_PRINT("evt(%d) (+%x, %d)\n", user_map_info->evt_reg,
			   cipc_get_offset((enum cipc_region)user_map_info->evt_reg),
			   cipc_get_size((enum cipc_region)user_map_info->evt_reg));

		for (j = 0; j < user_map_info->data_pool_cnt; j++)
			CIPC_PRINT("data(%d) (+%x, %d)\n", user_map_info->data_reg[j],
			   cipc_get_offset((enum cipc_region)user_map_info->data_reg[j]),
			   cipc_get_size((enum cipc_region)user_map_info->data_reg[j]));

		if (pre_user_map_info) {
			if ((cipc_get_offset((enum cipc_region)pre_user_map_info->reg) +
			     cipc_get_size((enum cipc_region)pre_user_map_info->reg)) >
			    cipc_get_offset((enum cipc_region)user_map_info->reg))
				CIPC_PRINT
				    ("CIPC invalid map: pre_offset:+%x, pre_size:%d, > cur_offset:+%x\n",
				     cipc_get_offset((enum cipc_region)pre_user_map_info->reg),
				     cipc_get_size((enum cipc_region)pre_user_map_info->reg),
				     cipc_get_offset((enum cipc_region)user_map_info->reg));
		}
...
static int cipc_set_map(int master)
{
	...
	cipc_user_ival = (struct cipc_user_map_info *)(cipc.chub_bootargs->cipc_init_map 
                                                      + (unsigned int)cipc.sram_base);
	...
	for (userid = (enum cipc_user_id)0; userid < CIPC_USER_MAX; userid++, cipc_user_ival++) {
		...
		if (cipc_user_ival->reg) {
			...
			cipc_set_info((enum cipc_region)user_info->map_info.evt_reg,
			...
			for (j = 0; j < CIPC_USER_DATA_MAX; j++) {
				if (user_info->map_info.data_size[j]) {
					...
					cipc_set_info((enum cipc_region)user_info->map_info.data_reg[j], ipc_data,
			...
			cipc_set_info((enum cipc_region)user_info->map_info.reg, user_map,
			...
			if (userid == CIPC_USER_CHUB2AP) {
				...
				if (cipc_get_offset_addr(user_map_base + next_user_pool_off) % IPC_ALIGN_SIZE)
					CIPC_PRINT
					    ("%s: aline_check(%d) error: base:+%x, remain:%d\n",
					    __func__,
				...
				CIPC_PRINT
					("%s4: aline_check on last user of each owner(%d), remain:%d cipc_map->next_user_pool_off:%d\n",
			...
			int *size)
{
...
#endif
		if (user_info->map_info.reg != (unsigned int)user_to_reg((enum cipc_user_id)i)) {
			CIPC_PRINT("%s invalid reg:%d, %d\n", __func__,
				   user_info->map_info.reg, user_to_reg((enum cipc_user_id)i));
			return CIPC_ERR;
		}
		if (user_info->map_info.owner == owner) {
			if (!onwer_start_off) {
				CIPC_PRINT("%s: update#1 start_off: +%x\n",
					   __func__,
					   cipc_get_offset((enum cipc_region)user_info->
							   map_info.reg));
				onwer_start_off =
				    cipc_get_offset((enum cipc_region)user_info->map_info.reg);
			}

			if (cipc_get_offset((enum cipc_region)user_info->map_info.reg) < onwer_start_off) {
				CIPC_PRINT
			...
			owner_size += cipc_get_size((enum cipc_region)user_info->map_info.reg);
			user = (void *)cipc_get_base((enum cipc_region)user_info->map_info.reg);
			CIPC_PRINT
			    ("%s loop(%d-%s) find: user-magic:%x, owner_id:%d, owner(addr:+%x, size:%d), cur-user(addr:+%x, size:%d)\n",
			     ...
			     cipc_get_offset((enum cipc_region)user_info->map_info.reg), cipc_get_size((enum cipc_region)user_info->map_info.reg));
		}
	}
	...
	if (onwer_start_off) {
		...
		*size = owner_size + (IPC_ALIGN_SIZE - owner_size % IPC_ALIGN_SIZE);
		...
		if (owner != IPC_OWN_MASTER) {
			if (*start_off % IPC_ALIGN_SIZE || (*size % IPC_ALIGN_SIZE))
				CIPC_PRINT("%s: err start_offset(+%x,remain:%d) or size(%d,remain:%d) isn't 4KB aline.",
...
int cipc_register(void *mb_addr, enum cipc_user_id tx, enum cipc_user_id rx,
		  ...
		  int *size)
{
	enum ipc_owner my_owner_id = (enum ipc_owner)cipc.user_info[rx].map_info.owner;
	enum ipc_mb_id my_mb_id = (enum ipc_mb_id)cipc.user_info[rx].map_info.dst.mb_id;
...
int cipc_reset_map(void)
{
	...
	for (i = 0; i < cipc.user_cnt; i++) {
		...
		/* step: set user magic */
		reg = (enum cipc_region)user_info->map_info.reg;
			    ...
			    struct cipc_funcs *funcs)
{
	...
	/*set cipc start */
	cipc.chub_bootargs = (struct chub_bootargs_cipc *)((char *)cipc.sram_base + MAP_INFO_OFFSET);
	cipc.cipc_map = (struct cipc_map_area *)((char *)cipc.sram_base + cipc.chub_bootargs->ipc_start + CIPC_START_OFFSET);

	/* step: set sram_base into local cipc addr */
	cipc_set_info(CIPC_REG_SRAM_BASE, sram_base, 0, "sram_base");
	cipc_set_info(CIPC_REG_IPC_BASE, (char *)cipc.sram_base + cipc.chub_bootargs->ipc_start,
	...
	for (i = 0; i < CIPC_REG_MAX; i++) {
		if (is_user((enum cipc_region)i)) {
			cipc_total_size += cipc_get_size((enum cipc_region)i);
			CIPC_PRINT("%s: add user(%d) size:%d -> cipc_total_size:%d\n",
				__func__, i, cipc_get_size((enum cipc_region)i), cipc_total_size);
		}
	}
	cipc_set_info(CIPC_REG_CIPC_BASE, cipc.cipc_map, cipc_total_size, "cipc_base");
	CIPC_PRINT("%s: owner:%d, sram_base:+%x, ipc_base:+%x, cipc_base:+%x\n", __func__, owner,
		   cipc_get_offset(CIPC_REG_SRAM_BASE),
		   cipc_get_offset(CIPC_REG_IPC_BASE),
		   cipc_get_offset(CIPC_REG_CIPC_BASE));
...
static void cipc_evt_fail_dbg(struct cipc_evt *ipc_evt, enum cipc_region reg)
{
	...
	if (is_user_valid(userid)) {
		...
		CIPC_PRINT("%s: evt(0x%x) (empty:%d,full:%d,eq:%d,dq:%d). user:%d(%s), reg:%d, SR:0x%x\n",
			...
			reg, ipc_hw_read_int_status_reg_all(user->mb_base, (enum ipc_mb_id)user->map_info.dst.mb_id));
	} else {
...
int cipc_add_evt(enum cipc_region reg, unsigned int evt)
{
	...
	user = &cipc.user_info[userid];
	my_mb_id = (enum ipc_mb_id)user->map_info.src.mb_id;
	opp_mb_id = (enum ipc_mb_id)user->map_info.dst.mb_id;
			...
			for (i = 0; i < IRQ_NUM_EVT_END; i++) {
				if (!ipc_evt->ctrl.pending[i]) {
					if (!ipc_hw_read_int_status_reg
						...
						CIPC_RAW_WRITEL((ipc_evt->ctrl.qctrl.eq + 1) % IPC_EVT_NUM,
...
{
	unsigned int gr, cr, mr, sr, msr;
        
	CIPC_PRINT("MB_SFR : mcutrl - 0x%x\n",
		   (unsigned int)CIPC_RAW_READL((char *)base + REG_MAILBOX_MCUCTL));
        
        gr = (unsigned int)CIPC_RAW_READL((char *)base + REG_MAILBOX_INTGR0);
        cr = (unsigned int)CIPC_RAW_READL((char *)base + REG_MAILBOX_INTCR0);
        mr = (unsigned int)CIPC_RAW_READL((char *)base + REG_MAILBOX_INTMR0);
        sr = (unsigned int)CIPC_RAW_READL((char *)base + REG_MAILBOX_INTSR0);
        msr = (unsigned int)CIPC_RAW_READL((char *)base + REG_MAILBOX_INTMSR0);
	CIPC_PRINT("MB_SFR0: 0x%x 0x%x 0x%x 0x%x 0x%x\n",gr, cr, mr, sr, msr);
        
        gr = (unsigned int)CIPC_RAW_READL((char *)base + REG_MAILBOX_INTGR1);
        cr = (unsigned int)CIPC_RAW_READL((char *)base + REG_MAILBOX_INTCR1);
        mr = (unsigned int)CIPC_RAW_READL((char *)base + REG_MAILBOX_INTMR1);
        sr = (unsigned int)CIPC_RAW_READL((char *)base + REG_MAILBOX_INTSR1);
        msr = (unsigned int)CIPC_RAW_READL((char *)base + REG_MAILBOX_INTMSR1);
...
void cipc_dump(enum cipc_user_id id)
{
	...
	for (i = (reg + 1); i < (reg + 3); i++) {
		CIPC_PRINT("%s: reg:%d, is_evt:%d, is_data:%d, is_user:%d\n",
			__func__, i, is_evt((enum cipc_region)i), is_data((enum cipc_region)i), is_user((enum cipc_region)i));
		if (is_evt((enum cipc_region)i))
			ipc_print_evt((enum cipc_region)i);
		else if (is_data((enum cipc_region)i))
			ipc_print_databuf((enum cipc_region)i);
	}
...
int cipc_irqhandler(enum cipc_user_id id, unsigned int status)
{
	...
	if (!is_evt_valid((enum cipc_region)user->map_info.evt_reg)) {
		CIPC_PRINT("%s: it's not a cipc evt\n", __func__);
		return CIPC_ERR;
	}

	start_index = ipc_hw_start_bit((enum ipc_mb_id)user->map_info.dst.mb_id);
	cipc_evt = cipc_get_base((enum cipc_region)user->map_info.evt_reg);
	org_status = status;

	if (!status) {
		status =
		    ipc_hw_read_int_status_reg_all(user->mb_base,
						   (enum ipc_mb_id)user->map_info.dst.mb_id);
	}
	/* ipc interrupt handle */
	while (status) {
		cur_evt = cipc_get_evt((enum cipc_region)user->map_info.evt_reg);
		if (cur_evt) {
			evt = cur_evt->evt;
			...
			if (!cipc_evt->ctrl.pending[cur_evt->mb_irq]
				...
				CIPC_PRINT
				    ("%s: trigger-err(sw:%d,hw:%d), irq:%d(%d + %d), evt:%d, status:0x%x->0x%x(SR:0x%x)\n",
				     ...
				     (user->mb_base, (enum ipc_mb_id)user->map_info.dst.mb_id));
...
int cipc_loopback_test(int reg_val, int start)
{
	...
	enum cipc_region reg_reply = (enum cipc_region)0;
	int i;
	int ret;
	unsigned int len;
		...
		if (local_ipc_testbuf.baaw_test) {
			for (i = CIPC_REG_IPC_BASE; i >= CIPC_REG_SRAM_BASE; i--) {
				CIPC_PRINT("Baaw_test(reg:%d): write '0xbb00' on unaccess area: Base: +%x (val:%x)\n",
					i, cipc_get_offset((enum cipc_region)i), CIPC_RAW_READL(cipc_get_base((enum cipc_region)i)));
				CIPC_RAW_WRITEL(0xbb00, cipc_get_base((enum cipc_region)i));
