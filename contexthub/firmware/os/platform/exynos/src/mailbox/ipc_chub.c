...
{
	return (u32)ipc.ipc_addr[area].base - (u32)ipc.sram_base;
}

static u32 ipc_get_offset_addr(void *addr)
{
	return (u32) ((u8 *)addr - (u8 *)ipc.sram_base);
}
...
#if defined(DEF_SUPPORT_CPU_UTILIZATION)
void ipc_update_pginfo(u32 pgCnt, u64 pgTime, u64 curTime)
{
    ipc.ipc_map->cpu_info.pgcnt = pgCnt;
    ipc.ipc_map->cpu_info.pgtime = pgTime;
    ipc.ipc_map->cpu_info.curtime = curTime;
}

void ipc_update_cginfo(u32 cgCnt, u64 cgTime, u64 curTime)
{
    ipc.ipc_map->cpu_info.cgcnt = cgCnt;
    ipc.ipc_map->cpu_info.cgtime = cgTime;
    ipc.ipc_map->cpu_info.curtime = curTime;
}
#endif
