...
static PwmTimerInfoType mPwmTimerInfo[PWM_TIMER_NUM_MAX] = {
...
static struct sfrData mPwmSfrData[] = {
    {REG_PWM_TCFG0,      0},
    {REG_PWM_TCFG1,      0},
    {REG_PWM_TCON,       0},
    {REG_PWM_TCNTB0,     0},
...
int pwmEnableInterrupt(PwmTimerNumType timer )
{
    ...
    NVIC_EnableIRQ((IRQn_Type)mPwmTimerInfo[timer].irqNum);
...
int pwmDisableInterrupt(PwmTimerNumType timer )
{
    ...
    NVIC_DisableIRQ((IRQn_Type)mPwmTimerInfo[timer].irqNum);
    return 0;
}
...
void pwmSaveState(void)
{
    ...
    size = sizeof(mPwmSfrData) / sizeof(struct sfrData);

    for (idx = 0 ; idx < size ; idx++) {
        mPwmSfrData[idx].val = __raw_readl(mPwmSfrData[idx].addr);
    }
}
...
void pwmRestoreState(void)
{
    ...
    for (idx = 0 ; idx < size ; idx++) {
        __raw_writel(mPwmSfrData[idx].val, mPwmSfrData[idx].addr);
    }
}
#endif
...
int pwmSetTime(PwmTimerNumType timer, uint32_t uSecond )
{
    ...
    if( mSec > PWM_MAX_MS || uSecond == 0) {
        CSP_PRINTF_ERROR("pwmSetTime: Invalid time(%lu ms %lu us)\n", mSec, uSec);
        //CSP_ASSERT(0);
        return -1;
    }
