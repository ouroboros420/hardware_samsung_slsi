...
void sysregSetMaskIrq(IN uint64_t irq)
{
    ...
    regValue = __raw_readl(REG_SYSREG_HWACG_CM4_CLKREQ) & 0xE0000000;
    regValue |= (uint32_t)irq&SYSREQ_CHUB_HWACG_CM4_CLKREQ_MASK_IRQ;
    __raw_writel(regValue, REG_SYSREG_HWACG_CM4_CLKREQ);

    regValue |= (uint32_t)(irq>>28);
    __raw_writel(regValue, REG_SYSREG_HWACG_CM4_CLKREQ1);
}
...
uint64_t sysregGetMaskIrq(void)
{
    ...
    regValue = __raw_readl(REG_SYSREG_HWACG_CM4_CLKREQ1);
    regValue = regValue << 28;
    regValue |= __raw_readl(REG_SYSREG_HWACG_CM4_CLKREQ) & SYSREQ_CHUB_HWACG_CM4_CLKREQ_MASK_IRQ;
