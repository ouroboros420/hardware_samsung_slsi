...
bool ContextHub::Flash(const std::string& filename) {
    std::unique_ptr<FILE, decltype(&fclose)> firmware_file(fopen(filename.c_str(), "r"), &fclose);
    if (!firmware_file) {
        LOGE("Failed to open firmware image: %d (%m)", errno);
        return false;
    }

    fseek(firmware_file.get(), 0, SEEK_END);
    long file_size = ftell(firmware_file.get());
    if (file_size < 0) {
        LOGE("failed to get ftell() at SEEK_END: %m");
        return false;
    }
    fseek(firmware_file.get(), 0, SEEK_SET);

    std::vector<uint8_t> firmware_data(file_size);
    size_t bytes_read = fread(firmware_data.data(), sizeof(uint8_t), file_size, firmware_file.get());
