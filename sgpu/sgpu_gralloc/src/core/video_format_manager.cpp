...
#undef LOG_TAG
#define LOG_TAG "VideoFormatManager"
...
struct video_format_layout_info {
        PixelFormat   format;
        uint32_t      layouts;
};

static const video_format_layout_info video_format_layout_table[] = {
        { PixelFormat::RGBA_8888,                           layout_linear },
        { PixelFormat::RGBX_8888,                           layout_linear },
        { PixelFormat::RGB_888,                             layout_linear },
        { PixelFormat::RGB_565,                             layout_linear },
        { PixelFormat::BGRA_8888,                           layout_linear },
        { PixelFormat::YCBCR_422_SP,                        layout_linear },
        { PixelFormat::YCRCB_420_SP,                        layout_linear },
        { PixelFormat::BLOB,                                layout_linear },
        { PixelFormat::YCBCR_420_888,                       layout_linear },
        { PixelFormat::RGBA_1010102,                        layout_linear },
        { PixelFormat::YCBCR_P010,                          layout_linear },
        { PixelFormat::R_8,                                 layout_linear },
        { PixelFormat::Y8,                                  layout_linear },
        { PixelFormat::Y16,                                 layout_linear },
        ...
        { PixelFormat::PRIVATE_YCBCR_420_SP,                layout_linear },
...
{
        constexpr uint32_t count = SGR_ARRAY_SIZE(video_format_layout_table);
        for (uint32_t i = 0; i < count; i++) {
                if (format == video_format_layout_table[i].format)
                        return i;
        }
        return -1;
}
...
///
uint32_t VideoFormatManager::get_layout(PixelFormat format, const BufferDescriptor &descriptor) const
{
        SGR_UNUSED(descriptor);

        const int32_t index = get_video_format_index(format);
        if (index == -1) {
                SGR_LOGE("Unsupported Format = %d", static_cast<uint32_t>(format));
                return 0;
        }

        const uint32_t layout = video_format_layout_table[static_cast<uint32_t>(index)].layouts;
...
align_info VideoFormatManager::get_linear_alignment(PixelFormat format) const
{
        ...
        if (format == PixelFormat::YV12) {
                ...
                align.vstride_in_pixels = 1;
        } else if (format == PixelFormat::PRIVATE_YCBCR_420_SP_M_S10B ||
        ...
        dump_alignment(align, static_cast<uint32_t>(IpBitMask::VIDEO));
