...
enum class PixelFormat : int32_t {
        ...
        PRIVATE_YCBCR_420_SPN_256_SBWC      = 0x136,
        PRIVATE_YCBCR_420_SPN_10B_256_SBWC  = 0x137,
        /* SBWC Lossy formats */
        ...
        PRIVATE_YCBCR_420_SP_M_32_SBWC_L     = 0x180,
        PRIVATE_YCBCR_420_SP_M_64_SBWC_L     = 0x181,
        PRIVATE_YCBCR_420_SPN_32_SBWC_L      = 0x182,
        PRIVATE_YCBCR_420_SPN_64_SBWC_L      = 0x183,
        PRIVATE_YCBCR_420_SP_M_10B_32_SBWC_L = 0x184,
...
struct BufferDescriptorInfo {
        hardware::hidl_string name;
        uint32_t width;
        uint32_t height;
        uint32_t layerCount;
        PixelFormat format __attribute__ ((aligned(4)));
        uint64_t usage;
        ...
        std::string dump() const
        {
                ...
                str << "BufferDescriptorInfo : " << this << "\n";
                str << "\t\t name            = " << (this->name)                          << "\n";
                str << "\t\t width           = " << (this->width)                         << "\n";
                str << "\t\t height          = " << (this->height)                        << "\n";
                str << "\t\t layerCount      = " << (this->layerCount)                    << "\n";
                str << "\t\t format          = " << (static_cast<uint64_t>(this->format)) << "\n";
                str << "\t\t usage           = " << (this->usage)                         << "\n";
                str << "\t\t reservedSize    = " << (this->reservedSize)                  << "\n";
...
struct BufferDescriptor {
        ...
        {
                std::stringstream str;
                std::string formatted_name(this->name, this->length_name);
                str << "BufferDescriptor  : " << this << "\n";
                str << "\t\t length_name  = " << (this->length_name)                   << "\n";
                str << "\t\t name         = " << (formatted_name)                      << "\n";
                str << "\t\t width        = " << (this->width)                         << "\n";
                str << "\t\t height       = " << (this->height)                        << "\n";
                str << "\t\t layerCount   = " << (this->layerCount)                    << "\n";
                str << "\t\t format       = " << (static_cast<uint64_t>(this->format)) << "\n";
...
struct YCbCrLayout {
        ...
        {
                std::stringstream str;
                str << "YCbCrLayout : " << this << "\n";
                str << "\t\t y          = " << (this->y)          << "\n";
                str << "\t\t cb         = " << (this->cb)         << "\n";
                str << "\t\t cr         = " << (this->cr)         << "\n";
                str << "\t\t yStride    = " << (this->yStride)    << "\n";
                str << "\t\t cStride    = " << (this->cStride)    << "\n";
                str << "\t\t chromaStep = " << (this->chromaStep) << "\n";
...
struct MetadataTypeDescription {
    ...
    const hardware::hidl_string description;
    bool is_gettable;
    bool is_settable;
};
