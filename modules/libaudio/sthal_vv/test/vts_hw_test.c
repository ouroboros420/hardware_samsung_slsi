...
#define SOUND_MODEL_ODMVOICE_BINARY           "/data/voice_dva_svoice.bin"
...
int set_dmic_ctrls(int flag)
{
    ...
    if (vtsMixerHandle) {
        ...
        if (!dmic_usagecnt) {
            ...
            printf("%s: %s MIC Controls ", __func__, flag ? "Enable\n" : "Disable\n");
        }
...
func_exit:
    ...
    if (vts_dev->thread_exit == false) {
        ...
        set_mixer_ctrls(vtsMixerHandle, model_recognize_stop_ctlname,
                odmvoice_triggerrecognize_stop_ctlvalue, MODEL_STOP_CONTROL_COUNT, false);
...
void vts_recognition_start(struct vts_hw_device *vts_dev, int model)
{
    ...
    if(model == ODM_MODEL)
        strcpy(model_bin, SOUND_MODEL_ODMVOICE_BINARY);
    else
    ...
    if (vts_dev->vts_state == VTS_RECOGNITION_STOPPED_STATE) {
        ...
        if(model == ODM_MODEL)
            set_mixer_ctrls(vtsMixerHandle, model_recognize_start_ctlname,
                odmvoice_triggerrecognize_start_ctlvalue, MODEL_START_CONTROL_COUNT, false);
        else
        set_mixer_ctrls(vtsMixerHandle, model_recognize_start_ctlname,
...
void vts_recognition_stop(struct vts_hw_device *vts_dev)
{
    ...
    if (vts_dev->vts_state == VTS_RECOGNITION_STARTED_STATE) {
        ...
        if(vts_dev->vts_model_type == ODM_MODEL)
            set_mixer_ctrls(vtsMixerHandle, model_recognize_stop_ctlname,
                odmvoice_triggerrecognize_stop_ctlvalue, MODEL_STOP_CONTROL_COUNT, false);
        else
            set_mixer_ctrls(vtsMixerHandle, model_recognize_stop_ctlname,
...
func_exit:
    if (vts_dev->odmrsvd2_thread_exit == false) {
        /* once callback thread is closed set vts_stop using sysfs */
        set_mixer_ctrls(vtsMixerHandle, model_recognize_stop_ctlname,
                odmvoice_reserved2recognize_stop_ctlvalue, MODEL_STOP_CONTROL_COUNT, false);
...
void vts_odmrsvd2_start(struct vts_hw_device *vts_dev)
{
    ...
    if (vts_dev->vts_odmrsvd2_state == VTS_RESERVED2_STOPPED_STATE) {
        ...
        set_mixer_ctrls(vtsMixerHandle, model_recognize_start_ctlname,
                        odmvoice_reserved2recognize_start_ctlvalue, MODEL_START_CONTROL_COUNT, false);
...
void print_options(struct vts_hw_device *vts_dev __unused)
{
    ...
    printf("6. ODMRsvd2 Mode Start\n");
    printf("7. ODMRsvd2 Mode Stop\n");
    printf("8. Exit - VTS Test Application\n");
    printf("****************************************************************!\n");
...
int main(void)
{
    ...
    while (1) {
        ...
        case 2:
            ...
            break;
        case 3: /* Stop loaded sound Model Recognition */
            vts_recognition_stop(vts_dev);
            break;
        case 4: /* Start VTS recording */
            vts_record_start(vts_dev);
            break;
        case 5: /* Stop VTS recording */
            vts_record_stop(vts_dev);
            break;
        case 6:
