...
void gpioInit(void)
{
    ...
    for (pin = GPIO_CHUB_START_PIN_NUM ; pin < GPIO_CHUB_MAX_PIN_NUM ; pin++) {
        ...
        NVIC_DisableIRQ(mGpioPinInfo[pin].irqNum);
        NVIC_ClearPendingIRQ(mGpioPinInfo[pin].irqNum);
    }
}
...
static int gpioCheckHandler(const struct Gpio* __restrict gpio)
{
    unsigned int lr;
    __asm__ __volatile__ ("mov %0, lr" : "=r" (lr));

    if (gpio == NULL) {
        PRINTF_ERROR("Invalid GPIO Handler (LR:0x%X)\n", lr);
        return -1;
    }
    return 0;
}
...
void gpioEnableInterrupt(GpioPinNumType gpioPinNum, uint32_t en)
{
    ...
    if (en) {
        ...
        if (gpioPinNum < GPIO_CHUB_MAX_PIN_NUM) {
            NVIC_ClearPendingIRQ(mGpioPinInfo[gpioPinNum].irqNum);
            NVIC_EnableIRQ(mGpioPinInfo[gpioPinNum].irqNum);
        }
    } else {
        regValue |= regMask;
        if (gpioPinNum < GPIO_CHUB_MAX_PIN_NUM) {
            NVIC_DisableIRQ(mGpioPinInfo[gpioPinNum].irqNum);
            NVIC_ClearPendingIRQ(mGpioPinInfo[gpioPinNum].irqNum);
        }
    }
...
//hack    cpuRegisterHandler(mGpioPinInfo[gpioPinNum].irqNum, callbackFunction);
    mpuAllowRomWrite(true);
    vectorBase = vectorBase + 0x40 + (mGpioPinInfo[gpioPinNum].irqNum * 4);
    *(uint32_t *)vectorBase = (uint32_t)callbackFunction;
...
{
    GpioPinNumType pin;
    for (pin = 0 ; pin < GPIO_CHUB_MAX_PIN_NUM ; pin++) {
        gpioUnsetExtInterrupt(pin);
    }
}
...
{
    if (gpioIsPending(mGpioPinInfo[gpioPinNum].pin)) {
...
{
    if (gpioCheckHandler(gpio))
        return;

    gpioUnsetExtInterrupt(gpio->pin);

    if (gpio->pin < GPIO_CHUB_MAX_PIN_NUM) {
        NVIC_ClearPendingIRQ(gpio->irqNum);
        NVIC_DisableIRQ(gpio->irqNum);
    }
}

void gpioConfigInput(const struct Gpio* __restrict gpio, int32_t drvStrength, enum GpioPullMode pull)
{
    ...
    gpioSetFunction(gpio->pin, GPIO_CON_INPUT);
    gpioSetPud(gpio->pin, pud);
    gpioSetDrvStrength(gpio->pin, drvStrength);
}

void gpioConfigOutput(const struct Gpio* __restrict gpio, int32_t drvStrength, enum GpioPullMode pull, enum GpioOpenDrainMode odrMode, bool value)
{
    uint32_t pud = GPIO_PUD_PULLDOWN;
    ...
    gpioSetPud(gpio->pin, pud);
    gpioSetDrvStrength(gpio->pin, drvStrength);
    gpioSetData(gpio->pin, value);
    gpioSetFunction(gpio->pin, GPIO_CON_OUTPUT);
}

void gpioConfigAlt(const struct Gpio* __restrict gpio, int32_t drvStrength, enum GpioPullMode pull, enum GpioOpenDrainMode odrMode, uint32_t altFunc)
{
    uint32_t pud = GPIO_PUD_PULLDOWN;
    ...
    gpioSetPud(gpio->pin, pud);
    gpioSetDrvStrength(gpio->pin, drvStrength);
    gpioSetFunction(gpio->pin, altFunc);
}

void gpioSetInterrupt(struct Gpio* __restrict gpio,
                         IntTriggerType intTrigger,
                         IntFilterSel   intFilter,
                         uint32_t       filterWidth,
                         void (*callbackFunction) (void))
{
    ...
    gpioSetExtInterrupt(gpio->pin, intTrigger, intFilter, filterWidth, callbackFunction);
}
