...
static void debugSetIPCLoopback(enum ipc_debug_event event)
{
    ...
    } else if (event == IPC_DEBUG_UTC_IPC_TEST_END) {
        mLoopbackTest = 0;
...
void debugHandler(void)
{
    ...
    case IPC_DEBUG_UTC_WDT:
		...
		__raw_readl(WDT_BASE_ADDRESS), __raw_readl(WDT_BASE_ADDRESS + 4));
        while(wdt_val > 0) {
	    wdt_val++;
	    if (wdt_val == 10000000) {
...
void saveNvicRegister(void) {
    struct ipc_map_area *map = ipc_get_base(IPC_REG_IPC);
    struct chub_persist* persist = &map->persist;
    if (20 > CHUB_PERSISTBUF_SIZE) {
        *((u32*)persist->fault_handler) = 0xcafecafe;
    } else {
        *((u32*)persist->fault_handler) = NVIC->ISER[0];
        *((u32*)(persist->fault_handler+4)) = NVIC->ICER[0];
        *((u32*)(persist->fault_handler+8)) = NVIC->ISPR[0];
        *((u32*)(persist->fault_handler+12)) = NVIC->ICPR[0];
        *((u32*)(persist->fault_handler+16)) = NVIC->IABR[0];
    }
}
