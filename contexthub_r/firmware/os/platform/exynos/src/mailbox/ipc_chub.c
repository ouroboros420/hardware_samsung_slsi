...
static void ipc_set_info(enum ipc_region area, void *base, int size, char *name)
{
	...
	strncpy(ipc.ipc_addr[area].name, name, IPC_NAME_MAX);
}
...
extern struct memlog_obj *memlog_file_chub;
extern struct memlog_obj *memlog_obj_chub;
extern struct memlog_obj *memlog_arr_chub;
		...
		while (eq != logbuf->dq) {
			...
			if (len > 0 && len <= LOGBUF_DATA_SIZE) {
				...
				if (log->level || log->timestamp) {
					switch (log->level) {
						case 'E':
							level = MEMLOG_LEVEL_ERR;
							break;
						case 'W':
							level = MEMLOG_LEVEL_CAUTION;
							break;
						case 'I':
							level = MEMLOG_LEVEL_INFO;
							break;
						default:
							level = MEMLOG_LEVEL_DEBUG;
							break;
					}

					sprintf(buf, "[%6llu.%06llu]%c %s",
							(log->timestamp)/1000000,
							(log->timestamp)%1000000,
							log->level, (char *) log->buf);
				} else {
					data.nextline = true;
					sprintf(buf, "                  %s", (char *) log->buf);
				}

				if (memlog_obj_chub)
					memlog_write(memlog_obj_chub, 1, buf, strlen(buf));

				if (memlog_arr_chub) {
					data.timestamp_chub = log->timestamp;
					memcpy(data.string, log->buf, strlen(log->buf));
					ret = memlog_write_array(memlog_arr_chub, 1, &data);
					if (ret < 0)
						CSP_PRINTF_ERROR("memlog array fail");
				}
				if (rt_buf) {
					if (rt_buf->write_index + len +
					    LOGFILE_NUM_SIZE >=
					    rt_buf->buffer_size)
						rt_buf->write_index = 0;
					lenout =
					    snprintf(rt_buf->buffer +
						     rt_buf->write_index,
						     LOGFILE_NUM_SIZE + len,
						     "%10d:%s\n", log->size,
						     log);
					rt_buf->write_index += lenout;
					if (lenout != (LOGFILE_NUM_SIZE + len))
						CSP_PRINTF_DEBUG
						    ("%s: %s: size-n missmatch: %d -> %d\n",
						     NAME_PREFIX, __func__,
						     LOGFILE_NUM_SIZE + len,
						     lenout);
				}
