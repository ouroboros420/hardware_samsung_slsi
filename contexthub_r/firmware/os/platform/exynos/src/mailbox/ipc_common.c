		  ...
		  int *size)
{
	...
	CIPC_PRINT("%s: owner:%d done: printout cipc local information: start_offset:+%x, size;%d\n",
...
{
	if ((caller == IPC_DBG_AP_SLEEP) || (caller == IPC_DBG_AP_WAKE) || (ipc_get_ap_wake() == AP_SLEEP)) {
