...
void i2c_IRQHandler(uint32_t busId)
{
    ...
    if ((mI2cInfo[busId].msg.flags & I2C_M_RD) && (intStat & (I2C_INT_RX_ALMOSTFULL_EN | I2C_INT_TRAILING_EN))) {
    ...
    if (mI2cInfo[busId].msg.flags & I2C_M_RD) {
        ...
        if (mI2cInfo[busId].stop == 0) {
            NVIC_DisableIRQ(mI2cInfo[busId].irqNum);
...
static void i2cInitTiming(uint32_t busId)
{
    uint32_t pClk;
    uint32_t opClk;
    uint32_t clk_div, tscl_h, tstart_hd, stop2idle;
    uint32_t regValue;
...
static void i2cMasterReset(uint32_t busId)
{
    ...
    pinSCL = mI2cInfo[busId].gpioSCL;
    pinSDA = mI2cInfo[busId].gpioSDA;
    ...
    PRINTF_INFO("SDA line:%d, SCL line:%d\n", sdaVal, sclVal);
...
static int i2cCheckIdle(uint32_t busId)
{
    ...
    PRINTF_INFO("I2C Bus is not IDLE: TRANS_STATUS = %x\n", (unsigned int)(regValue));
...
static int i2cMasterTransferPolling(uint32_t busId)
{
    ...
    ret = -I2C_ERR_TIMEOUT;
    regValue = sysregGetMaskIrq();
    regValue |= I2C_INTERRUPT_MASK;
    sysregSetMaskIrq(regValue);
...
static int i2cMasterTransfer(uint32_t busId)
{
    ...
    regValue =  __raw_readl(mI2cInfo[busId].sfrBase + REG_I2C_CONF2);
    regValue |= I2C_MASTER_RUN;
    __raw_writel(regValue, mI2cInfo[busId].sfrBase + REG_I2C_CONF2);

    if (mI2cInfo[busId].operationMode == I2C_MODE_INTERRUPT) {
        // Enable IRQ
        NVIC_EnableIRQ(mI2cInfo[busId].irqNum);
    } else {
