...
#define BMP280_I2C_BUS_ID               BARO_I2C_BUS_ID
...
#ifdef EXYNOS_CONTEXTHUB
#define I2C_ADDR_SDO_L                  0x76
#define I2C_ADDR_SDO_H                  0x77
#define I2C_ADDR                        I2C_ADDR_SDO_L
#else
...
#define BOSCH_BMP280_REG_STATUS         0xf3
...
#define VERBOSE_PRINT(fmt, ...) do { \
	if (enable_verbose) {  \
        osLog(LOG_VERBOSE, " [BMP280] " fmt, ##__VA_ARGS__); \
    }\
	}while (0);

#define INFO_PRINT(fmt, ...) do { \
        osLog(LOG_INFO, " [BMP280] " fmt, ##__VA_ARGS__); \
    } while (0);

#define ERROR_PRINT(fmt, ...) do { \
        osLog(LOG_ERROR, " [BMP280] " fmt, ##__VA_ARGS__); \
    } while (0);
...
static bool sensorCalibrateBaro(void *cookie)
{
    ...
    if (mTask.baroOn || mTask.tempOn) {
        ERROR_PRINT("cannot calibrate while baro or temp are active\n");
...
static void printCompParmas()
{
    int i;
    DEBUG_PRINT("\n-----compParams debug-----\n");
    DEBUG_PRINT("dig_T1:%d\n", (int)mTask.comp.dig_T1);
    DEBUG_PRINT("dig_T2:%d\n", (int)mTask.comp.dig_T2);
    DEBUG_PRINT("dig_T3:%d\n", (int)mTask.comp.dig_T3);
    DEBUG_PRINT("dig_P1:%d\n", (int)mTask.comp.dig_P1);
    DEBUG_PRINT("dig_P2:%d\n", (int)mTask.comp.dig_P2);
    DEBUG_PRINT("dig_P3:%d\n", (int)mTask.comp.dig_P3);
    DEBUG_PRINT("dig_P4:%d\n", (int)mTask.comp.dig_P4);
    DEBUG_PRINT("dig_P5:%d\n", (int)mTask.comp.dig_P5);
    DEBUG_PRINT("dig_P6:%d\n", (int)mTask.comp.dig_P6);
    DEBUG_PRINT("dig_P7:%d\n", (int)mTask.comp.dig_P7);
    DEBUG_PRINT("dig_P8:%d\n", (int)mTask.comp.dig_P8);
    DEBUG_PRINT("dig_P9:%d\n", (int)mTask.comp.dig_P9);

    for(i=0;i<24;i++)
    {
        DEBUG_PRINT("REG 0x%X : %u", BOSCH_BMP280_REG_DIG_T1+i, ((uint8_t*)&mTask.comp)[i]);
...
static void handleI2cEvent(struct I2cTransfer *xfer)
{
    ...
    switch (xfer->state) {
        ...
        case STATE_VERIFY_ID: {
            ...
            newXfer = allocXfer(STATE_WAIT_READY);
            if (newXfer != NULL) {
                newXfer->txrxBuf[0] = BOSCH_BMP280_REG_STATUS;
                ret = i2cMasterTxRx(I2C_BUS_ID, I2C_ADDR, newXfer->txrxBuf, 1, newXfer->txrxBuf, 1, i2cCallback, newXfer);
                if (ret)
                    releaseXfer(newXfer);
            }
        ...
        case STATE_FINISH_INIT: {
            INFO_PRINT("detected\n");
            ipc_set_sensor_info(SENSOR_TYPE_PRESSURE, NULL, NULL, SENS_TYPE_BARO, BOSCH_BMP280_ID);
                ...
                if (mTask.baroCalibrating) {
                    ...
                    if (baroAllocateEvt(&baroSample, pressure_Pa * 0.01f, sensorGetTime())) {
                        if (!osEnqueueEvtOrFree(EVENT_TYPE_BIT_DISCARDABLE | sensorGetMyEventType(SENS_TYPE_BARO), baroSample, baroFreeEvt)) {
                            INFO_PRINT("failed to enqueue baro sample\n");
                        }
                    }
                }
            }
...
{
#ifdef EXYNOS_CONTEXTHUB
    CSP_PRINTF_INFO("BMP280: task starting\n");
#endif
    mTask.id = taskId;
    mTask.offset = 0.0f;
...
#ifdef EXYNOS_CONTEXTHUB
    ipc_set_sensor_info(SENSOR_TYPE_PRESSURE, "BMI280 Press", VENDOR_NAME_BOSCH,  0, 0);
#endif
    return true;
}
