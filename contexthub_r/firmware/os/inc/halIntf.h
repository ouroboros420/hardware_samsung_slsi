...
// buffer data structure with header, header compatible with HostIntfDataBuffer
SET_PACKED_STRUCT_MODE_ON
struct DfsToSensorHalDataPayload {
    union {
        uint32_t sensType;
        uint32_t request;
    };
    union {
        struct DfsInfo dfsInfo;
        uint32_t data[FREQ_MAX];
    };
};
SET_PACKED_STRUCT_MODE_OFF
...
// buffer data structure with header, header compatible with HostIntfDataBuffer
SET_PACKED_STRUCT_MODE_ON
typedef struct DfsToSensorHalDataBuffer {
    uint32_t eventType; // placeholder for HostIntfDataBuffer event type field
    struct DfsToSensorHalDataPayload payload;
}DfsMsg;
SET_PACKED_STRUCT_MODE_OFF
