				  ...
				  size_t buf_len)
{
	...
	if (os_strcasecmp(cmd, "MACADDR") == 0) {
		...
		if (os_strncasecmp(cmd, "SET_PMK", 7) == 0)
			/*
			 * For SET_PMK Command, total length of 41 should be used.
			 * Length of "SET_PMK "(8) + PMK_LEN (32) + 1
			 */
			os_memcpy(buf, cmd, 41);
		else
...
int wpa_driver_set_p2p_noa(void *priv, u8 count, int start, int duration)
{
	...
	/* Driver Ioctl allows configuration of NoA interval and start_time in addition to
	 * count and duration as control interface command p2p set NoA configures start_time
	 * whereas driver private command configures interval in addition to count and
	 * duration.
	 * P2P_SET_NOA <count> <interval> <duration> <start> */
	snprintf(buf, sizeof(buf), "P2P_SET_NOA %d %d %d %d", count, 0, duration, start);
...
int wpa_driver_set_p2p_ps(void *priv, int legacy_ps, int opp_ps, int ctwindow)
{
	...
	/* 'value' < 10 (should be 0); suggests opp_ps is disabled; else opp_ps is enabled and ct_window is valid */
	if (value < 10)
		snprintf(buf, sizeof(buf), "P2P_SET_PS %d %d %d", 0, value, 0);
	else
		snprintf(buf, sizeof(buf), "P2P_SET_PS %d %d %d", 0, 1, value);
